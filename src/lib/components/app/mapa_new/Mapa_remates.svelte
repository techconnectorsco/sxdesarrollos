<script lang="ts">
    ...
    // 🔴 CAMBIO CRÍTICO: Solo pintamos en los Tilesets las fincas 
    // que el servidor SÍ encontró en la base de datos (las que tienen geometría)
   ...
   function registrarInteraccionParcela(datos: any) {
        // 1. Guardar en LocalStorage (inmediato) para que la página de detalle lo lea
        localStorage.setItem('parcelaData', JSON.stringify(datos));

        // 2. Enviar a BD (Asíncrono)
    ...
    export function highlightProperty(prop: any) {
        ...
        for (const [key, config] of Object.entries(DISTRICT_CONFIG)) {
            ...
            const features = map.querySourceFeatures(sourceId, {
                ...
                // Aquí quitamos el filtro estricto y filtramos en JS abajo,
                // o intentamos un filtro 'in' amplio si es posible.
                // Para hover rápido, iteramos los features devueltos:
            });

            // ✅ Búsqueda flexible en los features visibles
    ...
    // 1. Asegurar que el contenedor sea visible (útil en móviles)
    if (mapContainer) {
        mapContainer.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }

    // 2. SOLO navegamos si tiene geometría validada desde la DB
    if (prop.geometria) {
        try {
            ...
            console.error("❌ Error al procesar la geometría para FlyTo:", e);
        }
    } else {
        // 3. Si no tiene geometría, no buscamos en Tilesets. 
        // Avisamos discretamente por consola o podrías poner un toast de "Ubicación no disponible"
        console.warn(`📍 El remate ${prop.finca_id} no tiene ubicación exacta validada.`);
    }
}
    ...
    function loadTilesets(addListeners = true, isSatellite = false) {
        ...
        Object.entries(DISTRICT_CONFIG).forEach(([key, config]) => {
            ...
            /* if (key === 'cantondes-cr-alypap') {
                ...
                // C. Capa de Símbolos (Texto NOMB_UGEC)
                ...
                // D. AGREGAR EVENTO CLICK ESPECÍFICO PARA ESTA CAPA
                if (addListeners) {
                    map.on('click', clickLayerId, (e) => {
                        if (!e.features || e.features.length === 0) return;
                        const props = e.features[0].properties;
                        console.log("📍 Propiedades del Cantón:", props);
                        new mapboxgl.Popup()
                            .setLngLat(e.lngLat)
                            .setHTML(`
                                <div style="color:black; padding:5px;">
                                    <strong>Cantón:</strong> ${props?.NOMB_UGEC || 'Sin nombre'}<br>
            ...
            */
            ...
            // ============================================================
            // 🚨 CASO ESPECIAL: DISTRITOS (Visualización + Click de Atributos)
            // ============================================================
            // Descomenta este bloque para habilitar la visualización de Distritos
            
            if (['distrito1', 'distrito2', 'distrito3'].includes(key)) {
                const labelLayerId = `label-${key}`;
                const clickLayerId = `click-fill-${key}`; // Capa invisible para clicks

                // A. Capa de Línea (Bordes) - Color diferente a cantones (ej: Azul oscuro)
                if (!map.getLayer(lineLayerId)) {
                    map.addLayer({
                        id: lineLayerId,
                        type: 'line',
                        source: sourceId,
                        'source-layer': config.sourceLayer,
                        minzoom: 8, // Un poco más cerca que cantones
                        paint: {
                            'line-color': '#FF0000', 
                            'line-width': 1,         // Más fino que cantones
                ...
                // C. Capa de Símbolos (Texto: NOMB_UGED)
                if (!map.getLayer(labelLayerId)) {
                    map.addLayer({
                        id: labelLayerId,
                        type: 'symbol',
                        source: sourceId,
                        'source-layer': config.sourceLayer,
                        minzoom: 12, // Mostrar nombre al acercarse más
                        layout: {
                ...
                // D. AGREGAR EVENTO CLICK ESPECÍFICO PARA DISTRITOS
                if (addListeners) {
                    map.on('click', clickLayerId, (e) => {
                        if (!e.features || e.features.length === 0) return;
                        const props = e.features[0].properties;
                        
                        // console.log(`📍 Propiedades del Distrito (${key}):`, props);
                ...
                // 🛑 RETURN: Importante para evitar lógica de ventas
                return; 
            }
   ...
   function handleSelectFeature(feature: any, fillLayerId: string, lineLayerId: string, markerId: string, lngLat: mapboxgl.LngLat) {
        ...
        const provinciaTileset = props.PROVINCIA; // Asumiendo que el tileset trae el código (1-7)
        const cantonTileset = props.CANTON;       // Asumiendo que el tileset trae el código (01-XX)

        limpiarSeleccion();

        // ✅ USAMOS LA NUEVA LÓGICA FLEXIBLE
        // Esto busca en 'propiedadesActivas' usando Finca + Provincia + Cantón
    ...
    // Helper auxiliar para obtener el source ID dado un layer ID (convención tuya: fill-X -> source-X)
    ...
    function mostrarPopupCatastral(props: any, lngLat: mapboxgl.LngLat, feature?: any, layerId?: string) {
        ...
        console.log(`🔍 Tileset Remates -> Finca: ${fincaNum} | Prov: ${provinciaCode}`);
        ...
        <div class="bg-white/90 backdrop-blur border border-gray-200 shadow-xl rounded-full px-6 py-2 flex items-center gap-3 animate-in fade-in slide-in-from-bottom-4">
            <div class="flex items-center gap-2">
                ...
                {#if propiedadesConGeometria.length < totalRemates}
                    ...
                    <span class="text-xs px-2 py-1 bg-amber-100 text-amber-800 rounded-full font-semibold" title="Solo {propiedadesConGeometria.length} de {totalRemates} tienen ubicación en el mapa">
                        ⚠️ {propiedadesConGeometria.length}/{totalRemates}
                    </span>
                {:else}
                    <span class="text-sm font-bold text-gray-700">
