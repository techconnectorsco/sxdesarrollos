<script lang="ts">
	...
	const badge: Record<ClienteSAP['tipo'], string> = {
		...
		individual:
			'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 ring-slate-200 dark:ring-slate-700 ring-1 ring-inset'
	};
...
>
	<!-- ══ HERO DE MARCA ════════════════════════════════════════════════════ -->
	<div
		class="cxc-hero relative overflow-hidden"
		style="background-color: var(--brand-primary-light);"
	>
		<div class="h-1 w-full" style="background-color: var(--brand-primary)"></div>

		<div class="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
			<div
				class="absolute -right-24 -top-24 h-72 w-72 rounded-full opacity-[0.06]"
				style="background-color: var(--brand-primary)"
			></div>
			<div
				class="absolute -left-16 top-8 h-48 w-48 rounded-full opacity-[0.04]"
				style="background-color: var(--brand-primary)"
			></div>
			<div
				class="absolute right-1/3 -bottom-12 h-56 w-56 rounded-full opacity-[0.05]"
				style="background-color: var(--brand-primary)"
			></div>
		</div>
		...
		<div class="relative mx-auto max-w-5xl px-4 py-8">
			<div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
				<div class="flex items-center gap-5">
					{#if brand.logoUrl}
						<div
							class="cxc-hero-logo shrink-0 rounded-2xl border bg-white p-3 shadow-sm"
					...
					<div>
						<p
							class="cxc-hero-sub text-[11px] font-bold uppercase tracking-[0.2em]"
				...
				<div class="flex flex-wrap items-center gap-3 sm:flex-col sm:items-end">
					<div
						class="cxc-hero-badge flex items-center gap-2 rounded-xl border bg-white px-4 py-2 shadow-sm"
						style="border-color: var(--brand-primary-border);"
					>
						<span
						...
						></span>
						<span
							class="cxc-hero-badge-text text-xs font-bold uppercase tracking-wider"
					...
					<div
						class="cxc-hero-badge flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2 shadow-sm"
					>
						<span class="h-2 w-2 rounded-full bg-emerald-500"></span>
						<span class="cxc-hero-stat text-xs font-medium text-muted-foreground"
							>SAP · Conectado</span
						>
					</div>
				</div>
			...
			<div
				class="mt-6 h-px w-full rounded-full opacity-20"
	...
	<div class="mx-auto max-w-5xl px-4 py-8">
		...
		<section class="rounded-xl border border-border bg-card shadow-sm">
			...
			<div class="p-6">
				<div>
					<label for="metodo" class="mb-1 block text-xs font-medium text-muted-foreground"
						>Método</label
					>
					<select
						id="metodo"
				...
				{#if alcanceEfectivo === 'cliente'}
					<div class="relative mt-5">
						...
						{#if mostrarSugerencias && sugerencias.length > 0}
							<ul
								class="lista-sugerencias absolute z-10 mt-1 max-h-72 w-full overflow-auto rounded-lg border border-border bg-popover shadow-lg"
							>
								{#each sugerencias as c (c.cardCode)}
									<li>
										...
										>
											...
											<span
												class="shrink-0 rounded px-1.5 py-0.5 text-[10px] font-medium uppercase {badge[
													c.tipo
												]}"
												style={c.tipo === 'padre'
				...
				<div class="relative flex gap-2">
					<div class="relative flex-1">
						...
						{#if auditSugerencias && !auditSel && auditFiltrados.length > 0}
							<ul
								class="lista-audit absolute z-10 mt-1 max-h-72 w-full overflow-auto rounded-lg border border-border bg-popover shadow-lg"
							>
				...
				{#if auditData}
					<div class="mt-6">
						...
						<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
							<!-- Saldo Global en SAP -->
							<div
								class="rounded-xl border border-border bg-muted/50 p-4 border-l-4"
								style="border-left-color: var(--brand-primary);"
							>
								<p
									class="flex items-center gap-1 text-xs font-medium text-muted-foreground uppercase tracking-wider"
								>
									Saldo Global en SAP
									<span
										class="grid h-4 w-4 cursor-help place-items-center rounded-full bg-muted-foreground/20 text-[10px] font-bold normal-case text-muted-foreground"
										title="Balance total de la cuenta del cliente según SAP, consolidado en colones (moneda local). Los documentos en dólares se valoran en colones al tipo de cambio del día en que se registraron."
										>i</span
									>
								</p>
								<p
								...
								{#if auditData.cliente.saldoActual < 0}
									<span
										class="mt-1 inline-block rounded-md bg-amber-500 px-2 py-0.5 text-[10px] font-bold text-white"
										>Saldo a favor</span
									>
								{/if}
							</div>
							...
							>
								...
								<span
									class="rounded-full px-3 py-1 text-xs font-bold {envioHabilitado
						...
						<!-- Totales de documentos (FUERA del grid, fila propia) -->
										...
										<div class="rounded-xl border border-border bg-muted/50 p-4">
											<div class="flex items-baseline justify-between">
												...
												<span class="text-[11px] text-muted-foreground"
													>{auditData.documentos.usd.length} doc(s)</span
												>
											</div>
											<p
												class="mt-1 text-2xl font-bold tabular-nums {auditData.totales.usd < 0
													? 'text-amber-600'
													: 'text-foreground'}"
											>
												{fmt(auditData.totales.usd, 'USD')}
											</p>
										</div>
									{/if}
									{#if auditData.documentos.crc.length > 0}
										<div class="rounded-xl border border-border bg-muted/50 p-4">
											<div class="flex items-baseline justify-between">
												...
												<span class="text-[11px] text-muted-foreground"
													>{auditData.documentos.crc.length} doc(s)</span
												>
											</div>
											<p
												class="mt-1 text-2xl font-bold tabular-nums {auditData.totales.crc < 0
													? 'text-amber-600'
													: 'text-foreground'}"
											>
												{fmt(auditData.totales.crc, 'CRC')}
											</p>
										</div>
									{/if}
								</div>
						...
						{#if auditData.cliente.saldoActual === 0 && (auditData.documentos.usd.length > 0 || auditData.documentos.crc.length > 0)}
							...
							>
								...
								abiertos (probablemente facturas que se matan con notas de crédito no aplicadas). El
								envío automático no lo procesará.
							</div>
						{/if}
					</div>
					...
					<div class="mt-8 rounded-xl border border-border shadow-sm bg-card overflow-hidden">
						<div class="overflow-x-auto overflow-y-auto max-h-[500px] relative custom-scrollbar">
							<table class="w-full text-sm min-w-[900px] text-left">
								<thead
									class="text-[11px] font-bold uppercase text-white sticky top-0 z-10 shadow-sm"
									style="background-color: var(--brand-primary);"
								>
									<tr>
								...
								<tbody class="divide-y divide-border">
									{#if auditData.documentos.usd.length > 0}
										{#each auditData.documentos.usd as doc}
											...
											>
												...
												<td class="px-4 py-2.5 text-center">
													<span
														class="inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-bold {doc.estatus ===
														'Vencido'
										...
										{/each}
										<tr
											style="background-color: var(--brand-primary-light); color: var(--brand-primary-text);"
										>
											<td
												colspan="6"
												class="px-4 py-3 text-right text-xs font-bold tracking-wide uppercase"
						...
						<div class="mt-8 flex flex-wrap justify-center gap-6">
							{#each [{ m: 'USD', title: 'Dólares', data: auditData.rangos.usd }, { m: 'CRC', title: 'Colones', data: auditData.rangos.crc }] as rango}
								{#if rango.data.totalVencido > 0}
									...
									>
										<div
											class="px-4 py-3 text-center font-bold text-white"
											style="background-color: var(--brand-primary);"
										>
										...
										<div class="bg-card">
											...
											<div
												class="flex justify-between px-5 py-3 font-bold text-white"
												style="background-color: var(--brand-primary);"
											>
